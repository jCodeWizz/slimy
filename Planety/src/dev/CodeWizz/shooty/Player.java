...
public class Player {
	...
	private final float moveSpeed = 0.5f * mass, friction = -0.1f, bounceConstant = -0.5f, airFrictionX = 0.05f,
			airFrcitionY = 0.02f;
			...
			for (int i = 0; i < (int) speed.x; i++) {
				for (GameObject object : gc.handler.object) {
					if (collisionID.contains(object.getId())) {
						if (new Rectangle((int) position.x + 1, (int) position.y, (int) w, (int) h)
								.intersects(object.getBounds())) {
	...
	private void collisionX(GameObject object) {
		Vector v = new Vector(mass * object.getMass() * speed.x * -0.1f, 0);
		Vector v2 = new Vector(mass * object.getMass() * speed.x * 0.075f, 0);

		forces.add(v);
		object.getForces().add(v2);
	}

	private void collisionY(GameObject object) {
		Vector v = new Vector(0, mass * object.getMass() * speed.y * -0.1f);
		Vector v2 = new Vector(0, mass * object.getMass() * speed.y * 0.075f);
	...
	private void collisionY(GameContainer gc) {
		...
		if (speed.y >= 1) {
			for (int i = 0; i < (int) speed.y; i++) {
				for (GameObject object : gc.handler.object) {
					if (collisionID.contains(object.getId())) {
						if (new Rectangle((int) position.x, (int) position.y + 1, (int) w, (int) h)
								.intersects(object.getBounds())) {
							if (object.canMove()) {
								collisionY(object);
							}
	...
	public void render(GameContainer gc, Renderer r) {
		...
		// for(Vector vec : forces) {
		// r.drawLine(0xffff0000, (int) (position.x + 8), (int) (position.y + 8), (int)
		// (position.x + 8 + vec.x*5), (int) (position.y + 8 + vec.y*5));
		// }

		// r.drawLine(0xffffff00, (int) (position.x + 8), (int) (position.y + 8), (int)
		// (position.x + 8 + acc.x*200 ), (int) (position.y + 8 + acc.y*200));
